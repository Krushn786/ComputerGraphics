...
public class StopBeforeCollide : MonoBehaviour
{
  ...
  /*  protected void changeAgentDestination() {
        //print("INSIDE CHANGEDES");
        foreach (var n in greenAgent.ToArray())
        {
            if (n.isReached()) {
               // print("INSIDE CHANGEDES - if changed");

                agentReached.Add(n);
                n.isMoving();
            }
            foreach (var m in greenAgent.ToArray()) {
                //print("INSIDE CHANGEDES-for each change 1");

                foreach (var a in agentReached.ToArray()) {
                    //print("INSIDE CHANGEDES-for each change 2");
                    if (!(m.Equals(a)) && !m.isReached()) {
                        m.GetComponent<NavMeshAgent>().stoppingDistance = radiusTostop;
                    }
                }
            }
  ...
  */
    protected void changeAgentDest() {
        if (leader == null)
        {
            print("inside if");
            foreach (var n in greenAgent.ToArray())
            {
                ...
                if (n.isReached())
                {
                    ...
                    n.GetComponent<NavMeshAgent>().stoppingDistance = 0;
                    agentReached.Add(n);
                    leader = n;
                    print("leaderSelected" + n.GetComponent<NavMeshAgent>().name);
            ...
            if (!isStoppindDistanceSet)
            {
                foreach (var n in greenAgent.ToArray())
                {
                    //foreach (var m in agentReached.ToArray())
                    //{
                        if (!(leader.Equals(n)))
                        {
                            
                            //print("changed" +n.GetComponent<NavMeshAgent>().name);
                            n.GetComponent<NavMeshAgent>().stoppingDistance = 5f;
                        n.GetComponent<NavMeshAgent>().ResetPath();
                            // n.GetComponent<NavMeshAgent>().ResetPath();
                            //print("changed" + n.GetComponent<NavMeshAgent>().stoppingDistance);
                    }
                   // }
                }
                isStoppindDistanceSet = true;
            }
        }
