...
public class AgentManager : MonoBehaviour
{
    ...
    private void LeaderHelper()
    {
        float guessMass = 80f;
        Agent Alpha = null;


        var randomPostion = new Vector3((Random.value - 0.5f) * agentSpawnRadius, 0, (Random.value - 0.5f) * agentSpawnRadius);

            NavMeshHit hit;
            NavMesh.SamplePosition(randomPostion, out hit, 10, NavMesh.AllAreas);
            randomPostion = hit.position + Vector3.up;

            GameObject agent = null;
            agent = Instantiate(agentPrefab, randomPostion, Quaternion.identity);
            agent.name = "Leader";
            agent.transform.parent = agentParent.transform;
            var agentScript = agent.GetComponent<Agent>();
            agentScript.radius = 0.3f;// Random.Range(0.2f, 0.6f);
            agentScript.mass = guessMass;
            agentScript.perceptionRadius = 3;
